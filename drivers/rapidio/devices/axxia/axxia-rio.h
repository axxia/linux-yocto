...
#define RIO_SET_DID(size, x)					\
	((size) ? ((x) & 0xffff) : (((x) & 0x000000ff) << 16))
...
#define DESC_TABLE_W0_RAB_BASE(d)			\
	(RAB_REG_BASE + 0x10000 + DESC_TABLE_W0_NDX(d))
...
#define DESC_STATE_TO_ERRNO(s)					\
	((s) & DME_DESC_DW0_TIMEOUT_ERR ?			\
...
struct rio_priv {
	...
	struct mutex api_lock;	/* Lock for API Access */
