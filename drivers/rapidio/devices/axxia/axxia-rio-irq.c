				...
				u32 misc_state)
{
	...
	if (misc_state & AMST_INT) {
		...
		if (amast & RAB_AMAST_STAT_WRTO)
			dev_err(priv->dev,
				"AMST Write Response Timeout Error\n");
		if (amast & RAB_AMAST_STAT_RDTO)
			dev_err(priv->dev,
				"AMST Read Response Timeout Error\n");
				 ...
				 struct rio_desc **descriptors)
{
	...
	rdesc = kzalloc(sizeof(*rdesc) * entries, GFP_ATOMIC);
	if (!rdesc)
		goto err;
	rdescriptors = kzalloc(sizeof(*rdescriptors) * entries, GFP_ATOMIC);
	if (!rdescriptors)
		goto err;
	...
	if (dme_stat & (OB_DME_STAT_DESC_FETCH_ERR |
		...
		if (dme_stat & (OB_DME_STAT_DATA_TRANS_ERR |
			...
			if (dme_stat & OB_DME_STAT_RESP_TO)
				dev_err(priv->dev,
					"OB DME%d: Response Timeout Error\n",
					dme_no);
		}
			...
			*((u32 *)DESC_TABLE_W3_MEM(me, i)) = 0;
			...
			__rio_local_write_config_32(mport,
						    DESC_TABLE_W0(me->dres.start
								  + i), dw0);
			__rio_local_write_config_32(mport,
						    DESC_TABLE_W1(me->dres.start
								  + i), dw1);
			__rio_local_write_config_32(mport,
						    DESC_TABLE_W2(me->dres.start
								  + i), dw2);
			__rio_local_write_config_32(mport,
						    DESC_TABLE_W3(me->dres.start
								  + i), 0);
		}
	}
...
void axxia_close_outb_mbox(struct rio_mport *mport, int mbox_id)
{
	...
	if (!mb || !test_bit(RIO_MB_OPEN, &mb->state))
		return;
	me = mb->me;
